...
RISC-V (リスク-ファイブと発音する) は最初は学術研究用に開発された命令セットアーキテクチャ(ISA)であったが、
現在は産業界における無料でオープンなアーキテクチャの標準になることを期待されているアーキテクチャである。
RISC-Vの仕様を決めるにあたり、私たちは以下の目標を設定した:

\vspace{-0.1in}
\begin{itemize}
\parskip 0pt
\itemsep 1pt
\item 学術界と産業界において、誰でも入手できる完全に {\em オープン} なISAであること。
